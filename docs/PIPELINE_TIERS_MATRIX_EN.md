...
| Asset Pipeline | png/wav/cs/json/tmj/tsx/aseprite staging + 2D workflows | Tier A + 3D mesh/material workflows |
| Editor | 2D scene/tile tools | Unified editor with mode-aware tools |
| Runtime UI | Pixel UI/HUD | Pixel UI + hybrid UI variants |
| Export | Standalone export for 2D games, including WebAssembly handoff | Standalone export for 2D/3D/HD-2D |
