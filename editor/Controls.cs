...
namespace Balmung.Editor
{
    ...
    {
        // â”€â”€ Styled properties â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // â”€â”€ Internal controls â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        public LabeledField()
        {
            ...
            // TwoWay: TextBox â†’ Value (user edits)
        ...
        // â”€â”€ Avalonia property change hook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ ViewportPanel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Phase 3: placeholder with engine stats overlay.
    // Phase 4: replace with shared-context GL blit (add bm_engine_read_pixels to C-API).
        ...
        public ViewportPanel()
        {
            ...
            Child = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    new TextBlock
                    {
                        Text = "Phase 4: connect bm_engine_read_pixels\nor GL shared-context blit",
    ...
    public class SceneGridEditor : Control
    {
        ...
        public static readonly StyledProperty<GridToolMode> ToolModeProperty =
            AvaloniaProperty.Register<SceneGridEditor, GridToolMode>(nameof(ToolMode), GridToolMode.Select);
        ...
        private bool _linePending;
        private int _lineStartX = -1;
        private int _lineStartZ = -1;
        ...
        private void _onPointerPressed(object? sender, PointerPressedEventArgs e)
        {
            ...
            if (ToolMode == GridToolMode.Select)
            {
                _hoverX = x;
                _hoverZ = z;
                InvalidateVisual();
            ...
            _painting = true;
            _eraseDrag = e.GetCurrentPoint(this).Properties.IsRightButtonPressed || ToolMode == GridToolMode.Erase;
            _previousEraseMode = scene.EraseMode;
            _restoreEraseMode = _eraseDrag;
            if (_eraseDrag)
                scene.EraseMode = true;

            if (ToolMode == GridToolMode.Fill)
            {
                scene.FloodFillAt(x, z);
                _painting = false;
                if (_restoreEraseMode)
                    scene.EraseMode = _previousEraseMode;
                _restoreEraseMode = false;
            }
            else if (ToolMode == GridToolMode.Line)
            {
                if (!_linePending)
                {
                    _linePending = true;
                ...
                {
                    scene.DrawLine(_lineStartX, _lineStartZ, x, z);
                    _linePending = false;
                    _lineStartX = _lineStartZ = -1;
                }
                _painting = false;
                if (_restoreEraseMode)
                    scene.EraseMode = _previousEraseMode;
        ...
        private void _onPointerMoved(object? sender, PointerEventArgs e)
        {
            ...
            if (!_painting) return;
            if (ToolMode is GridToolMode.Paint or GridToolMode.Erase)
            {
